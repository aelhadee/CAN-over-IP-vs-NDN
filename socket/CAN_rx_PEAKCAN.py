...
while True:
    ...
    try:
        ...
        decrypted_msgs_matrix = aes_obj.decrypt_and_verify(CAN_bytes, tag)
        ...
        CAN_bytes = decrypted_msgs_matrix[:400]

    except:
        print('Not authentic or the received data is incomplete ')

    data = data[416:]  # store the bytes of the next new payload

    # write the logs of Delta time between "Data received" and "Data Decrypted"
    tcp_frame_number += 1
    max_frames = 500 * 1000  # stop at max and write the log
    ...
    MBps += (len(CAN_bytes + tag + data)) / (1000 * 1000)
    # print(len(rx_img))
    mbps1 = (MBps * 8)
